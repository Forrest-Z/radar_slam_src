...
public:
  FeatureExtrace() :nh("~") {
    ...
    nh.getParam("max_segment_doppler_diff", MAX_SEGMENT_DOPPLER_DIFF);
  ...
  void pointcloud_callback(const sensor_msgs::PointCloud2ConstPtr& arbe_origin_ros)
  {
    ...
    double duration_segment = (double)(end_segment - start) / CLOCKS_PER_SEC;
    double duration_publish = (double)(end_publish - start) / CLOCKS_PER_SEC;
        ...
        point_search_ind, point_search_sq_dis) > 0)
      {
        for (int j = 0;j < point_search_ind.size();j++)
        {
          ...
          if (point_label[search_index] == 0 && point_label_tmp[search_index] == 0)
          {
            ...
            if (doppler_diff < MAX_SEGMENT_DOPPLER_DIFF | (azimuth_diff < abs(azimuth_res * 1.5) && elevation_diff < abs(elevation_res * 1.5)))
            {
